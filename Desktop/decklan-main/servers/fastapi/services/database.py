...
import os
container_db_path = os.getenv("APP_DATA_DIRECTORY", "/tmp/decklan")
container_db_file = os.path.join(container_db_path, "container.db")
# Ensure 4 slashes for absolute path in SQLAlchemy
if container_db_file.startswith("/"):
    container_db_url = f"sqlite+aiosqlite:///{container_db_file}"  # 4 slashes for absolute path
else:
    container_db_url = f"sqlite+aiosqlite:///{os.path.abspath(container_db_file)}"
