...
TEMPLATE_PATH = os.path.join(os.path.dirname(__file__), "..", "..", "ai", "template.json")
...
# from anthropic import AsyncAnthropic  # Commented out per Jay's Week 4 Tuesday instruction
# from anthropic.types import Message as AnthropicMessage  # Commented out per Jay's Week 4 Tuesday instruction
# from anthropic import MessageStreamEvent as AnthropicMessageStreamEvent  # Commented out per Jay's Week 4 Tuesday instruction
from enums.llm_provider import LLMProvider
from models.llm_message import (
    # AnthropicAssistantMessage,  # Commented out per Jay's Week 4 Tuesday instruction
    # AnthropicUserMessage,  # Commented out per Jay's Week 4 Tuesday instruction
...
from models.llm_tool_call import (
    # AnthropicToolCall,  # Commented out per Jay's Week 4 Tuesday instruction
    GoogleToolCall,
    LLMToolCall,
    OpenAIToolCall,
...
from utils.get_env import (
    # get_anthropic_api_key_env,  # Commented out per Jay's Week 4 Tuesday instruction
    # get_custom_llm_api_key_env,  # Commented out per Jay's Week 4 Tuesday instruction
    # get_custom_llm_url_env,  # Commented out per Jay's Week 4 Tuesday instruction
    # get_disable_thinking_env,  # Commented out per Jay's Week 4 Tuesday instruction
    get_google_api_key_env,
    # get_ollama_url_env,  # Commented out per Jay's Week 4 Tuesday instruction
    get_openai_api_key_env,
    # get_tool_calls_env,  # Commented out per Jay's Week 4 Tuesday instruction
...
class LLMClient:
    ...
    def enable_web_grounding(self) -> bool:
        # Only OpenAI and Google support web grounding
        return parse_bool_or_none(get_web_grounding_env()) or False

    # ? Disable thinking
    # def disable_thinking(self) -> bool:  # Commented out per Jay's Week 4 Tuesday instruction - only used by Custom provider
    ...
    def _get_client(self):
        match self.llm_provider:
            ...
            # case LLMProvider.ANTHROPIC:  # Commented out per Jay's Week 4 Tuesday instruction
            #     return self._get_anthropic_client()
            # case LLMProvider.OLLAMA:  # Commented out per Jay's Week 4 Tuesday instruction
            #     return self._get_ollama_client()
            # case LLMProvider.CUSTOM:  # Commented out per Jay's Week 4 Tuesday instruction
            #     return self._get_custom_client()
            case _:
                raise HTTPException(
                    status_code=400,
                    detail="LLM Provider must be either openai or google",
                )
    ...
    # def _get_anthropic_client(self):  # Commented out per Jay's Week 4 Tuesday instruction
    ...
    # def _get_ollama_client(self):  # Commented out per Jay's Week 4 Tuesday instruction
    #     return AsyncOpenAI(
    #         base_url=(get_ollama_url_env() or "http://localhost:11434") + "/v1",
    #         api_key="ollama",
    #     )

    # def _get_custom_client(self):  # Commented out per Jay's Week 4 Tuesday instruction
    ...
    # def _get_anthropic_messages(self, messages: List[LLMMessage]) -> List[LLMMessage]:  # Commented out per Jay's Week 4 Tuesday instruction
    #     return [
    ...
    # async def _generate_anthropic(  # Commented out per Jay's Week 4 Tuesday instruction
    #     self,
    #     model: str,
    #     messages: List[LLMMessage],
    #     max_tokens: Optional[int] = None,
    ...
    # async def _generate_ollama(  # Commented out per Jay's Week 4 Tuesday instruction
    #     self,
    #     model: str,
    #     messages: List[LLMMessage],
    #     max_tokens: Optional[int] = None,
    ...
    # async def _generate_custom(  # Commented out per Jay's Week 4 Tuesday instruction
    #     self,
    #     model: str,
    #     messages: List[LLMMessage],
    #     max_tokens: Optional[int] = None,
    ...
    # async def _generate_anthropic_structured(  # Commented out per Jay's Week 4 Tuesday instruction
    #     self,
    #     model: str,
    #     messages: List[LLMMessage],
    #     response_format: dict,
    ...
    # async def _generate_ollama_structured(  # Commented out per Jay's Week 4 Tuesday instruction
    #     self,
    #     model: str,
    #     messages: List[LLMMessage],
    #     response_format: dict,
    ...
    # async def _generate_custom_structured(  # Commented out per Jay's Week 4 Tuesday instruction
    #     self,
    #     model: str,
    #     messages: List[LLMMessage],
    #     response_format: dict,
...
async def generate_structured(
    ...
    - loads ai/template.json
    - injects systemPrompt + userPromptTemplate
    - enforces strict JSON schema
    - retries on malformed model output
    """

    # Load schema-first template.json
    template = load_template()
    system_prompt = template["systemPrompt"]
    user_template = template["userPromptTemplate"]
    schema = template["schema"]

    # Build final messages using schema-first structure
    full_messages = [
        LLMSystemMessage(role="system", content=system_prompt),
        LLMUserMessage(role="user", content=user_template),
        *messages,
    ]

    # Override the response format with strict schema
    response_format = schema
    strict = True

    parsed_tools = self.tool_calls_handler.parse_tools(tools)

    # Run structured generation with full schema safety
    content = None
    match self.llm_provider:
        case LLMProvider.OPENAI:
            content = await self._generate_openai_structured(
    ...
    if content is None:
        raise HTTPException(
            status_code=400,
            detail="LLM returned no structured content",
        )
