...
async def export_presentation_as_pptx(
    ...
    try:
        ...
        print(f"Exporting PPTX with {len(pptx_model.slides)} slide(s)")
        ...
        error_msg = f"Unexpected error during PPTX export: {str(e)}"
...
@PRESENTATION_ROUTER.post("/generateDeck")
async def generate_deck_schema_first(
    title: Annotated[str, Body()],
    idea: Annotated[str, Body()],
    goal: Annotated[str, Body()],
    audience: Annotated[str, Body()],
    tone: Annotated[str, Body()] = "professional",
    description: Annotated[Optional[str], Body()] = None,
):
    """
    Schema-first deck generation endpoint using template.json workflow.
    Directly calls LLM with schema-first template injection.
    Returns the structured JSON response matching template.json schema.
    """
    try:
        # Load template.json
        ...
        # Format userPromptTemplate with provided values
        # Use 'idea' as 'description' if description not provided
        description_value = description or idea
        
        if isinstance(user_prompt_template, dict):
            # Format each value in the dict template
            formatted_dict = {}
            for key, value_template in user_prompt_template.items():
                try:
                    formatted_dict[key] = str(value_template).format(
                        title=title,
                    ...
                    formatted_dict[key] = str(value_template)
            # Convert to a readable string format
            user_prompt_content = "\n".join([f"{k}: {v}" for k, v in formatted_dict.items()])
        else:
            # If it's a string template, format it directly
        ...
        # Build messages - generate_structured will inject system + user from template
        # So we just pass the formatted user content as additional context
        messages = [
            LLMUserMessage(role="user", content=user_prompt_content),
        ]
        
        # Call generate_structured - it will inject systemPrompt + userPromptTemplate + schema
        client = LLMClient()
        model = get_model()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Schema-first deck generation failed: {str(e)}"
        )
