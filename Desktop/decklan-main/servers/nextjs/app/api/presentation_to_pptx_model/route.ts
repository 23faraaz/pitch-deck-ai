...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    [browser, page] = await getBrowserAndPage(id, request);
    ...
    console.error("Error in presentation_to_pptx_model:", error);
    await closeBrowserAndPage(browser, page);
...
async function getBrowserAndPage(id: string, request?: NextRequest): Promise<[Browser, Page]> {
  ...
  // Construct the URL - use NEXT_PUBLIC_APP_URL if available, otherwise construct from request or use default
  let baseUrl = process.env.NEXT_PUBLIC_APP_URL;
  
  if (!baseUrl && request) {
    const host = request.nextUrl.host;
    // If host doesn't include a port, check if it's localhost and add port 3000
    // Otherwise use the host as-is (might be through nginx on port 80)
  ...
  const pdfMakerUrl = `${baseUrl}/pdf-maker?id=${id}`;
...
function getScreenshotsDir() {
  // Use TEMP_DIRECTORY env var or fallback to default temp directory
  const tempDir = process.env.TEMP_DIRECTORY || path.join(os.tmpdir(), "decklan");
