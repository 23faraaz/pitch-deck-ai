  ...
  const fetchAvailableModels = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Set default to first available model (typically gpt-4o-mini)
        onInputChange(data[0] || "gpt-4o-mini", "openai_model");
      } else {
        const message =
          response.status >= 500
            ? "Decklan backend isn't running on port 8000. Start it with `node start.js --dev` and try again."
            : "Failed to fetch models. Double-check your API key.";
  ...
  return (
    <div className="space-y-6">
      ...
      {modelsChecked && availableModels.length === 0 && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800">
            Can't find models. Check your API key and try again.
          </p>
        </div>
      )}
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-gray-500 flex items-center gap-2">
          <span className="block w-1 h-1 rounded-full bg-gray-400"></span>
          Gives the AI access to current web information.
        </p>
      </div>
    </div>
  );
