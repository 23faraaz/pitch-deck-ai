...
export default function LLMProviderSelection({
  ...
  useEffect(() => {
    const needsModelSelection =
      ...
      // (llmConfig.LLM === "ollama" && !llmConfig.OLLAMA_MODEL) ||  // Commented out per Jay's Week 4 Tuesday instruction
      // (llmConfig.LLM === "custom" && !llmConfig.CUSTOM_MODEL) ||  // Commented out per Jay's Week 4 Tuesday instruction
      // (llmConfig.LLM === "anthropic" && !llmConfig.ANTHROPIC_MODEL);  // Commented out per Jay's Week 4 Tuesday instruction
    ...
    const needsApiKey =
      ...
      // (llmConfig.LLM === "anthropic" && !llmConfig.ANTHROPIC_API_KEY) ||  // Commented out per Jay's Week 4 Tuesday instruction
      (llmConfig.IMAGE_PROVIDER === "pexels" && !llmConfig.PEXELS_API_KEY) ||
      (llmConfig.IMAGE_PROVIDER === "pixabay" && !llmConfig.PIXABAY_API_KEY);

    // const needsOllamaUrl = (llmConfig.LLM === "ollama" && !llmConfig.OLLAMA_URL);  // Commented out per Jay's Week 4 Tuesday instruction

    setButtonState({
      isLoading: false,
      isDisabled: needsModelSelection || needsApiKey, // || needsOllamaUrl,  // Commented out per Jay's Week 4 Tuesday instruction
      text: needsModelSelection ? "Add your API key and choose a model to continue" : needsApiKey ? "Please Enter API Key" : "Continue to upload", // : needsOllamaUrl ? "Please Enter Ollama URL"  // Commented out per Jay's Week 4 Tuesday instruction
      showProgress: false
    });
    ...
    <div className="h-full flex flex-col mt-10">
      ...
      <div className="p-2 rounded-2xl border border-gray-200">
        ...
        >
          <TabsList className="grid w-full grid-cols-2 bg-transparent h-10">
            ...
            {/* <TabsTrigger value="anthropic">Anthropic</TabsTrigger>  Commented out per Jay's Week 4 Tuesday instruction */}
            {/* <TabsTrigger value="ollama">Ollama</TabsTrigger>  Commented out per Jay's Week 4 Tuesday instruction */}
            {/* <TabsTrigger value="custom">Custom</TabsTrigger>  Commented out per Jay's Week 4 Tuesday instruction */}
          </TabsList>
        </Tabs>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 pt-0 custom_scrollbar">
        ...
        >
          ...
          {/* Anthropic Content - Commented out per Jay's Week 4 Tuesday instruction */}
          ...
          {/* Ollama Content - Commented out per Jay's Week 4 Tuesday instruction */}
          ...
          {/* Custom Content - Commented out per Jay's Week 4 Tuesday instruction */}
